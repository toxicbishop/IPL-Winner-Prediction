...
def build_win_prob_matrix(model, matches_df: pd.DataFrame, bbb_df: pd.DataFrame) -> dict:
    """Build a dict[team1][team2] = P(team1 beats team2) using the trained model."""
    from config import ACTIVE_TEAMS_2026
    from src.prediction.predict import build_matchup_features
    ...
    for t1 in ACTIVE_TEAMS_2026:
        for t2 in ACTIVE_TEAMS_2026:
            if t1 == t2:
                matrix[t1][t2] = 0.5
                continue
            feats = build_matchup_features(t1, t2, matches_df, bbb_df)
...
def predict_2026_monte_carlo() -> list:
    ...
    from config import PROCESSED_MATCHES_CSV, get_tournament_paths
    from src.models.ensemble_model import EnsembleModel
    from src.prediction.predict import rank_predictions
    ...
    win_prob_matrix = build_win_prob_matrix(model, matches_df, bbb_df)
    ...
    from src.prediction.predict import bayesian_update
...
if __name__ == "__main__":
    ...
    from src.prediction.predict import print_predictions
